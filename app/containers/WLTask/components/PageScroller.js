...
export default class PageScroller extends Component {
  ...
  _getPageForOffset(offset, diff) {
    const boxPos = Math.abs(offset / this._boxSizeInterval);
    let index;
  ...
  _runAfterMeasurements(width, height) {
    ...
    for (let i = 0; i < this.props.children.length; ++i) {
      ...
      viewsOpacity.push(
        new Animated.Value(i === this._currentPage ? 1 : this.props.blurredOpacity)
      );
    }
  ...
  render() {
    ...
    return (
      ...
      >
        <Animated.View style={[{ flex: 1 }, containerStyle]} {...this._panResponder.panHandlers}>
