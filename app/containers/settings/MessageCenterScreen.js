...
class MessageCenterScreen extends BaseComponent {
  ...
  _onRefresh = isShowLoading => {
    let showLoading =
      isShowLoading == undefined || isShowLoading ? true : false;
    if (!this.userToken || this.userToken === null) {
      return;
    }
  ...
  _renderItem = item => {
    return (
      <Item item={item} onPressItem={this._onPressItem.bind(this, item)} />
    );
  };
...
class Item extends PureComponent {
  ...
  render() {
    ...
    if (messageType == 1) {
      ...
      let content1 =
        transactionType == 1
          ? I18n.t("settings.sender")
          : I18n.t("settings.receiver");
      content =
        content1 + address.substr(0, 8) + "......" + address.substr(34, 42);
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.item,
          { backgroundColor: readStatus == 2 ? "white" : Colors.bg_blue }
        ]}
