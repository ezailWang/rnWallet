...
class RightDrawer extends BaseComponent {
  ...
  render() {
    ...
    itcWalletList.forEach((wallet, index) => {
      const isSelected = wallet.address.toLowerCase() == currentWallet.address.toLowerCase();
      itcWalletsView.push(
        <Item
          key={index}
          wallet={wallet}
          isSelected={isSelected}
          itemOnPress={() => _this.itcWalletOnPress(wallet)}
        />
      );
    });

    ethWalletList.forEach((wallet, index) => {
      const isSelected = wallet.address.toLowerCase() == currentWallet.address.toLowerCase();
      ethWalletsView.push(
        <Item
          key={index}
          wallet={wallet}
          isSelected={isSelected}
          itemOnPress={() => _this.ethWalletOnPress(wallet)}
        />
      );
    });
