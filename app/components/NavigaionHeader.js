...
class BlueHeader extends Component {
  ...
  render() {
    ...
    const nHeight = Layout.NAVIGATION_HEIGHT();
    return (
      <LinearGradient colors={['#32beff', '#0095eb', '#2093ff']} style={[{ height: nHeight }]}>
        <BackWhiteButton
          onPress={() => {
            navigation.goBack();
          }}
...
class BlackBgHeader extends PureComponent {
  ...
  render() {
    ...
    return (
      <View style={[styles.blackBgContainer, { height: nHeight }]}>
        <TouchableOpacity
        ...
        <View style={[styles.headerTitleBox, contentMarginTop]}>
          <Text style={styles.whiteTitle}>{text}</Text>
        </View>

        <TouchableOpacity style={[styles.headerButtonBox, contentMarginTop]} onPress={rightPress}>
          <Image style={styles.icon} resizeMode="center" source={rightIcon} />
        </TouchableOpacity>
      </View>
    );
  }
...
class TransparentBgHeader extends PureComponent {
  ...
  render() {
    ...
    return (
      <View style={[styles.transparentBgContainer, { height: nHeight }]}>
        <TouchableOpacity
          style={[styles.headerButtonBox, contentMarginTop]}
          onPress={() => {
            navigation.goBack();
...
class WhiteBgHeader extends PureComponent {
  ...
  render() {
    const { rightText, leftPress, navigation, text, rightPress, rightIcon } = this.props;
    const nHeight = Layout.NAVIGATION_HEIGHT();
    let contentMarginTop = { marginTop: 24 };
    if (Layout.DEVICE_IS_IPHONE_X()) {
      contentMarginTop = { marginTop: 48 };
    ...
    return (
      <View style={[styles.headerBgContainer, { height: nHeight }]}>
        <TouchableOpacity
